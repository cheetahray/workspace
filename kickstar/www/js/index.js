...
var successListen = function (message) {
    if (message == "bell")
    {
        ...
        //hello.initialize(myip + entry, 7777, successInternal, failureSet);
    }
    else if (entry < 254)
    {
        entry = entry + 1;
        ...
        hello.initialize(myip + entry, 7777, successFor, failureSet);
    }
    else {
        app.receivedEvent('deviceready');
        if (language == "zh-TW")
...
function onPrompt(results) {
    if (results.buttonIndex == 1) {
        ...
        if (entry != NaN && entry < 255) {
            hello.initialize(myip + entry, 7777, successSet, failureSet);
        }
    }
}
