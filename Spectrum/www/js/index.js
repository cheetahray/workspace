...
boxLa.addEventListener('touchend', function (e) {
    ...
    sendto("start1", finalcountdown.toString());
    e.preventDefault();
}, false);
...
boxSo.addEventListener('touchend', function (e) {
    ...
    sendto("start2", finalcountdown.toString());
    e.preventDefault();
}, false);
...
boxFa.addEventListener('touchend', function (e) {
    ...
    sendto("start3", finalcountdown.toString());
    e.preventDefault();
}, false);
...
boxMi.addEventListener('touchend', function (e) {
    ...
    sendto("start4", finalcountdown.toString());
    e.preventDefault();
}, false);
...
function onPrompt(results) {
    if (results.buttonIndex == 1) {
        ...
        {
            if (entry > 0 && entry < 10)
                hello.initialize(myip + "10" + entry.toString(), 5005, successSet, failureSet);
            else if (entry >= 10 && entry < 100)
                hello.initialize(myip + (100 + entry).toString(), 5005, successSet, failureSet);
            else
                hello.initialize(myip + "201", 33153, successSet, failureSet);
        }
    }
}
